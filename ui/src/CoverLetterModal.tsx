    ...
    <div className="modal-overlay cover-modal-overlay" onClick={props.onClose}>
      <div className="modal-content cover-modal" onClick={(event) => event.stopPropagation()}>
        <div className="modal-header">
          ...
          <button className="btn btn-outline btn-sm" onClick={props.onClose}><X size={16} /></button>
        </div>
        ...
        {editing ? (
          <textarea className="cover-editor" value={draft} onChange={(event) => setDraft(event.target.value)} rows={18} />
        ) : (
