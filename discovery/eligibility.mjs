...
export function classifyEligibility(job) {
  ...
  if (has(s, /\b(worldwide|anywhere|any country|globally remote|remote global|global remote|international contractor)\b/)) return ELIGIBILITY.WORLDWIDE;
  ...
  if (has(s, /\b(remote\s+(?:us|usa)|us only|usa only|united states only|must (?:live|reside|be based) in (?:the )?(?:us|usa|united states)|authori[sz]ed to work in (?:the )?(?:us|usa|united states)|us time zones? only)\b/)) return ELIGIBILITY.US_ONLY;
  if (has(s, /\b(remote\s+uk|uk only|united kingdom only|must (?:live|reside|be based) in (?:the )?uk)\b/)) return ELIGIBILITY.UK_ONLY;
  if (has(s, /\b(remote\s+canada|canada only|must (?:live|reside|be based) in canada)\b/)) return ELIGIBILITY.CANADA_ONLY;
  ...
  if (Array.isArray(job?.eligibleCountries) && job.eligibleCountries.filter(Boolean).length > 1) return ELIGIBILITY.COUNTRY_LIST;
