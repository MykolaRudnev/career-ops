...
export function readProviderHealth() {
  ...
  for(const [source,p] of Object.entries(SOURCE_POLICIES)) if(!rows.some(r=>r.source===source)) rows.push({source,name:source,...p,status:p.status || 'DEGRADED',jobsFetched:0,newJobs:0,duplicates:0,errors:[p.reason || 'Not scanned yet'],durationMs:0,lastSuccessfulScan:null});
